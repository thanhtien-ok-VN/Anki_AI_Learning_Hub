...
Keep this skill focused on technical visualization and review workflows. Route presentation-specific work to `frontend-slides`.

| User request pattern | Route |
|---|---|
| Diagram, architecture overview, diff/plan review, project recap, technical comparison table, visual plan | **Use this skill (`visual-explainer`)** |
| "Pitch deck from scratch", "convert PPT/PPTX", "show me style preview options" | **Route to `frontend-slides`** |
| "Turn this diff/plan/project recap into slides" (`--slides`) | **Stay in this skill** (slide mode for technical artifacts) |
...
- For shared slide primitives (viewport-fit baseline and core style tokens): read `../_shared/references/slide-core.md` when generating or reviewing slide output
